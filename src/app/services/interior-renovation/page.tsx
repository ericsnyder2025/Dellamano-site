...
// hero/sub-services/CTA layout if no published row is found. Same wrapper
// pattern as exterior-living/page.tsx.

export const revalidate = 86400;

const SLUG = "/services/interior-renovation";
const PLACEHOLDER_TITLE = `Interior Renovations & Additions — ${BUSINESS_NAME}`;
...
async function fetchPublishedPage(): Promise<PageRow | null> {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.error(`[interior-renovation] fetch error:`, error.message);
    return null;
  }
...
async function fetchPhotos(pageId: string): Promise<PhotoRow[]> {
  ...
  const { data, error } = await supabase
    ...
    .eq("page_id", pageId);
  if (error) {
    console.error(`[interior-renovation] photos fetch error:`, error.message);
    return [];
  }
...
function PlaceholderBody() {
  return (
    <>
      <Hero
        eyebrow="Interior Renovation · South Florida"
      ...
      <ServiceGallery slug="interior-renovation" />
...
export default async function InteriorRenovationPillar() {
  const page = await fetchPublishedPage();
  ...
  const subheading =
    ...
    "Whole-home renovations, kitchen remodels, additions, and custom builds — one licensed GC managing every trade.";
  ...
  return (
    <>
      ...
      <Hero
        eyebrow="Interior Renovation · South Florida"
        heading={page.h1 || "South Florida Interior Renovation Contractor"}
      ...
      <PillarSubServices
        ...
        eyebrow="Interior Renovation services"
        heading="What we build inside the house"
        subheading="Kitchens, whole-home remodels, additions, and ground-up custom builds — each has its own detailed guide."
      />
